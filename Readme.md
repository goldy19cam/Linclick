...
Une fois le formulaire validé et les données enregistrées en base, l'application ouvre automatiquement  la fenêtre **Portail de liens**, listant les ressources disponibles pour l'utilisateur.

### 2.4 Erreurs courantes

| Problème | Cause probable | Solution |
|---|---|---|
| Le formulaire ne se valide pas | Champ Téléphone mal formaté | Vérifier que la longueur du numero est de 9 chiffres minimum|
...
│   ├── formulaire.h
│   ├── formulaire.cpp
│   └── formulaire.ui (si Qt Designer utilisé)
├── portail/
│   ├── portail.h
│   └── portail.cpp
├── database/
...
- **Validation des champs** :
  - Nom : `QRegularExpressionValidator` avec une expression du type `^[A-Za-zÀ-ÿ\\s'-]+$`.
  - Téléphone : `QRegularExpressionValidator` avec une expression du type `^[0-9]{9,15}$`.
- **Horodatage** : `QDateTimeEdit` utilisé pour capturer ou afficher la date/heure de la demande.

### 3.3 Style (QSS)

L'application utilise une feuille de style QSS pour Un fond completement blanc;
...
| `nombre_demandes` | INTEGER | Nombre de demandes totales |
...
Sous Qt Creator, il suffit d'ouvrir directement le fichier `LINKCLICK.pro` et de lancer la build+execution (Ctrl+R).
...
- Passage à une base de données distante (MySQL/PostgreSQL) pour un usage multi-poste via QSqlDatabase mais avec un driver différent (QMYSQL au lieu de QSQLITE)..
